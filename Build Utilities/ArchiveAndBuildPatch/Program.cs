...
namespace ArchiveAndBuildPatch
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			// If there exists a "previous but one" version, build a patch from it
			// to this new version:
			if (archiveFolderManager.PreviousButOneVersion != null)
			{
				var patchBuilder = new PatchBuilder(archiveFolderManager.PreviousButOneVersion, installer, archiveFolderManager);
			...
			// If there exists a previous version, build a patch from it
			// to this new version:
			if (archiveFolderManager.PreviousVersion != null)
			{
				var patchBuilder = new PatchBuilder(archiveFolderManager.PreviousVersion, installer, archiveFolderManager);
				var patchPath = patchBuilder.BuildPatch();
				var patchWrapper = new PatchWrapper(patchPath);
				patchWrapper.Wrap();
			}
