...
bool FileExists(_TCHAR * pszDirectory, _TCHAR * pszFileName)
{
	bool fReturn = false;
	const int kcchBufLen = 2048;
	_TCHAR pszPath[kcchBufLen];
	_tcscpy_s(pszPath, kcchBufLen, pszDirectory);
	_tcscat_s(pszPath, kcchBufLen, _T("\\"));
	_tcscat_s(pszPath, kcchBufLen, pszFileName);
	FILE * f;
...
// of FW has been detected which can be upgraded, therefore we expect to find the .exe files,
...
#endif
		if (FileExists(pszInstallDir, _T("Flex.exe"))
			|| FileExists(pszInstallDir, _T("TE.exe")))
		{
