/*
TODO: Replace this file with a C# application, so that the call to PEParser.exe can be
deprecated and instead the IsDotNetAssembly() method in GenerateFilesSource can be used.
...
// See if a merge module source file name appears as a command line argument:
...
{
	// Test if the proposed destination file is already newer than the given Wix source.
	// If it is, then we will do nothing:
	...
	if (fso.FileExists(ProcessedMmFilePath))
	{
		var sourceFile = fso.GetFile(MmFilePath);
		var destFile = fso.GetFile(ProcessedMmFilePath);

		if (sourceFile.DateLastModified <= destFile.DateLastModified)
			return;
	}
	...
	var root = xmlFiles.selectSingleNode("//wix:Wix");
	var comment = xmlFiles.createComment("\r\n\r\n*** THIS IS AN AUTOMATICALLY GENERATED FILE.***\r\n\r\nDo not edit this file directly. If you want to make changes, edit:\r\n" + MmFilePath + "\r\n\r\n");
	xmlFiles.insertBefore(comment, root);
...
function AddSpecificRegInfo(FileNode, TallowCmd)
{
	...
	// Set up XML parser for Tallow output:
	...
	for (b = 0; b < RegNodes.length; b++)
	{
		...
		// Replace any occurrence of '[' and']' with escaped equivalent. This is done in two
