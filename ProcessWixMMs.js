...
function ProcessWixMM(MmFilePath)
{
	...
	for (i = 0; i < FileNodes.length; i++)
	{
		...
		{
			// don't process COM-wise if it's a .net assembly or explicitly not a COM assembly
			var AssemblyAttributeValue = FileNode.getAttribute("Assembly");
			var IsDotNet = ((AssemblyAttributeValue != null) && (AssemblyAttributeValue == ".net"));
			var IsWin32 = ((AssemblyAttributeValue != null) && (AssemblyAttributeValue == "win32"));

			// Try running Tallow on qualified files to get any registration data there may be:
			//  but only if the File/@Assembly attribute isn't explicitly set to '.net' or 'win32'
			//  for which calling Tallow (i.e. regsvr32) throws an error
			try
			{
				if (!IsDotNet && !IsWin32)
					AddRegInfo(FileNode);
			...
			// if (IsDotNet)
			//  you could have this restriction, but I'm not sure if all .Net assemblies
			//  are explictly marked as such. And calling RegAsm even on those other types doesn't
			//  crash like calling Regsvr32 on .net and win32 types does. So well allow it.
